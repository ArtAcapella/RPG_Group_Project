public class Character
{
  protected int sts, bos, qck, buf, tech, eyes, cool, lck, chrm, ice, inl, hst, hck, corp, str, snk;
  protected boolean laug, haug, remhk;
  /*sts - street smarts; bos - book smarts; qck - quick; buf - buff; tech - tech; eyes - eyes; cool - cool; lck - luck; chrm - charm; ice - ICE; inl - intelligence; hst - history; hck - hack; corp - corporate; snk - sneak; laug - leg augment; haug - hand augment; remhk - remote hacking; str - strength*/
  Character(int sts, int bos, int qck, int buf, int tech, int eyes, int cool, int lck, int chrm, int ice, int inl, int hst, int hck, int corp, int str, int snk, boolean laug, boolean haug, boolean remhk)
  {
    this.sts = sts;
    this.bos = bos;
    this.qck = qck;
    this.buf = buf;
    this.tech = tech;
    this.eyes = eyes;
    this.cool = cool;
    this.lck = lck;
    this.chrm = chrm;
    this.ice = ice;
    this.inl = inl;
    this.hst = hst;
    this.hck = hck;
    this.corp = corp;
    this.str = str;
    this.snk = snk;
    this.laug = laug;
    this.haug = haug;
    this.remhk = remhk;
  }
}