    ...
    while (isu != "return home")
    {
          ...
          while (isu.equalsIgnoreCase("no") || isu.equalsIgnoreCase("yes"))
          {
            ...
            else if (isu.equalsIgnoreCase("yes"))
            {
              ...
              floor = 1;
		              ...
		              if (floor >= 5) {
			              System.out.println("Whever this user is they had access to high-quality gear and some sort of special access. It seems they were hunting Rhyz for something.");
			              } 
		              if (floor >= 10) {
		            	  System.out.println("The signiture is tracable to an Calgati HQ in Angel City. The most common reference the remote netrunner used here was - interestingly enough - employee data. Empoloyee data on Rhyz.");
			              if (ace.bos >= 5) {
			            	  System.out.println("Definitely Calgati, possibly even a highriser or even Abutens himself. What makes Rhyz so important? One other bit of information stands out to you: the phrase \"OUT OF THE NIGHT\"");
			              }
		              if (floor >= 15) {
		            	  System.out.println("Based on some audio snipets and flagged words by this other netrunner, it seems Rhyz was being tracked for connections to MageLan. One other bit of information stands out to you: the phrase \\\"OUT OF THE NIGHT\\\"");
			              }
      ...
      } else if(isu.equalsIgnoreCase("old ferris wheel") || isu.equalsIgnoreCase("ferris wheel") || isu.equalsIgnoreCase("wheel") || isu.equalsIgnoreCase("ferris")) 
      {
        ...
        isu = ent.nextLine();
	        ...
	        if (car == 10) {
	        System.out.println("Climbing the metal bars you pull youself into car number 10, and find youself standing over the body of a woman wearing some type of military gear. ");
	        if (ace.sts > 8)
	        {
	        ...
	        if (ace.tech > 9)
	        {
	          ...
	          inventory.add("old ARIES gear");
	        	...
	        	System.out.println("You're not entirely sure what to do here. Its uncomfortable standing over a dead body, so you climb back down the ferris wheel.");
	        }
	        if (ace.corp > 8)
	        {
      ...
      } else if (isu.equalsIgnoreCase("carnival game") || isu.equalsIgnoreCase("game") || isu.equalsIgnoreCase("carnival"))
      {
    	  moves--;
    	  System.out.println("You look around the carnival game on the edge of the pier. You can't make out what game used to be played here, since everything from prizes to the machinery used seems to have been stolen.");
        if (ace.eyes > 8)
        {
        	System.out.println("[EYES] oddly -and unervingly- you notice a toy gun laying on the ground that looters must not have been interested in. You pick it up, and realize from the weight that this is definitely not a toy.\n HANDGUN added to inventory");
          inventory.add("handgun");
        }
        
      } else if (isu.equalsIgnoreCase("security cameras") || isu.equalsIgnoreCase("cameras") || isu.equalsIgnoreCase("camera") )
      {
    	  moves--;
    	  System.out.println("The secrurity cameras are high up, you'll have to climb it somehow.\n Try to climb? [yes / no]");
    	  isu = ent.nextLine();
        ...
        {
          System.out.println("You manage to climb up the rusty pins on the side of the pier's pillars and hop onto one that lets you reach the camera's port.\nJack in?");
            ...
            else if (ent.nextLine() == "yes")
            {
              ...
              // hacking game
              System.out.println("You slide your sinapsring from your arm into the port and brace youself for whatever malware some hooligans instaled for laughs, but none comes. You make your way deeper into its net... \nand deeper... \nand deeper...\n until finally you find the first floor of this net. ");
              System.out.println("You're in.");
              jackedIn = true;
              floor = 1;
		              while (jackedIn) {
		              ...
		              if (floor >= 5) {
			              System.out.println("From floor 6 you manage to collect a recent message, seemingly left floating deep in this net for some reason. it reads: OUT OF THE NIGHT");
			              } 
		              if (floor >= 10) {
		            	  System.out.println("An avatar appars before you just as you're disconnecting from the net.");
			              if (ace.bos >= 10) {
			            	  System.out.println("And not just any avatar, if memory serves this is the form the user MORI used when she blew open corpo security across the country for the July 4th Raids. Someone must have stolen this avatar for cred, since she was pronounced dead from a kill program.");
			              }
			              System.out.println("Talented, aren't you? Her voice reads. Its light and ephemeral, almost unnaturally so. Definitely a voice changer at work. You're seeking Rhyz, correct?");
			              ...
			              System.out.println("and with that the avatar disapears and you go to exit the security net.");
			              } 
		              if (floor >= 15) {
		            	  System.out.println("As you begin to leave the net, you realize something. That avatar was Mori. No, not just avatar. That signiture was MORI. the Mori who caused the independence day raids. the Mori thats presumed dead. How is she connected to Rhyz?!");
			              }
    		  ...
    		  System.out.println("You try to pull yourself up, but arent strong or agile enough to make it to the synapstring port.");
    	  }
        ...
        }else if (isu.equalsIgnoreCase("apartments") || isu.equalsIgnoreCase("pier ruined apartments") || isu.equalsIgnoreCase("ruined apartments")) {
        	moves--;
          if (ace.sts > 7)
          {
          ...
          while (inApartments) {
	          ...
	          if (isu.equalsIgnoreCase("122") || isu.equalsIgnoreCase("Room 122")) {
	        	  ...
	        	  if (isu.equalsIgnoreCase("1")) {
	        		  moves--;
	        		  if (ace.buf > 12) {
	        			  System.out.println("The door opens. You may now travel directly to the clinic from your appartment by typing \"CLINIC\"");
	        			  
	        			  
	        			  //END SCENE! RETURN SUCCESS! MOVE ON TO RHYZ CLINIC!
	        			  Clinic clinic = new Clinic(aBuff, aQuick, aTech, aHacking, aBookSmarts, aStreetSmarts, aDealing, aCool, aSneak, aDodge, aEyes, aCharm, aIntimidate, aLie, aHPmax, background);
      ...
      else if (isu.equalsIgnoreCase("mall") || isu.equalsIgnoreCase("abandoned mall"))
        {
          ...
          if (ace.snk > 9)
          {
            ...
            {
            	moves--;
                System.out.println("You try to get a greeting out but the guard is too jumpy to let you say another word once he sees your face in his territory.");
                if (ace.qck > 8)
                {
                  System.out.println("[Quick] Fortunately, you manage to flee before the guard opens fire.");
                }
                else
                {
                  System.out.println("Combat begins. Combat machine broke so lets just say you win. Just keep this broken part of the game between us, ok? great. Here, ill give you some extra moves if you keep quiet about this...");
            ...
            System.out.println("Past the guard's patrol, you make your way to the piles of tech the L34Ds have been using. Theres a local net theyve been linking everything to that you could maybe get into.");
            ...
            if (ace.remhk || inventory.contains("saw") || pastGuard == true)
            {
              ...
              floor = 1;
		              ...
		              //Netrun rewards
		              if (floor >=4) {
		            	  System.out.println("Security camera records indicate Rhyz is somewhere on the first floor between room 110 and 123");
		              } if (floor >=8) {
		            	  System.out.println("The L34Ds have been contracted by someone, seemingly by the name of Abutens.");
		            	  if (ace.bos >= 8) {
		            		  System.out.println("Abutens is the name of the CEO of Calgati. Why does he have an interest in Rhyz? And why did he atatch this note for the L34Ds saying \"Password likely is: OUT OF THE NIGHT\"");
		            	  }
		              } if (floor >=12) {
		            	  System.out.println("It seems based on the notes of the L34D stalkers that Rhyz is connected to Magelan somehow.");
		              }
              ...
              if (ace.tech > 6)
	              {
	                System.out.println("You rig the L34D's router to explode as a distraction. You move to a safe distance and let the explosion happen, then run the other way as you hear footseps approaching.");
	              } else {
	            	  System.out.println("you can't figure out where the volatile components are hooked in the net, so rigging this isn't going to work.");
	              }
              }
	              ...
	              if (ace.hck > 10)
	              {
	                System.out.println("You play police sirens in the mall.");
	            	  ...
	            	  System.out.println("Whoever put this net together created such a mess you're not sure if its even connected to speakers, much less where the imput for them is.");
	              }
            }
