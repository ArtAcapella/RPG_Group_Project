...
class Combat {
	...
	static int combatMethod(Character enemy, int aBuff, int aQuick, int aTech, int aHacking, int aEyes, int aSneak,
		...
		// ie
		/*
		 * if movechoice.equalsignorecase"assess" { if aEyes > 5 { println(weakPoint1)
		 * if enemy.buff < aBuff { enemyName + "'s appears very physically weak'" } } if
		 * aEyes > 10 { println(weakPoint2) } if aEyes > 15 { println(weakPoint3) } if
		 * aEyes > 20 { println(weakPoint4) } }
		 * 
		 * if movechoice.equalsignorecase"grapple" { if aBuff > enemy.Buff { "you pin "
		 * enemyName "in your hold" current enemy AC = current enemy AC - aBuff;
		 * 
		 * 
		 * } else { enemy.name + "breaks out of your hold" } }
		 */
